...
bafEmissionFromMatrix2 <- function(BAF,
	...
	computeEmitPr <- function(dlist, prOutlier, allele.prob){
		...
		p <- matrix(prOutlier, nr, nc, byrow=TRUE)
		t(sumd+p)
	}
	##---------------------------------------------------------------------------
	## copy number 0
	emission[, , 1] <- dunif(BAF, 0, 1)

	##---------------------------------------------------------------------------
	## copy number 1
	trNormal2 <- trnorm2(BAF)
	dA <- trNormal2(mus[,"A"], sds[, "A"])
	dB <- trNormal2(mus[,"B"], sds[, "B"])
	##tdA <- t(dA)
	##tdB <- t(dB)
	emission[, , 2] <- computeEmitPr(list(dA,dB),
					 prOutlier[2, ],
					 allele.prob[, c("A", "B")])

	##---------------------------------------------------------------------------
	## copy number 2
	dAB <- trNormal2(mus[, "AB"], sds[, "AB"])
	#tdAB <- t(dAB)
	emission[, , 3] <- computeEmitPr(list(dA, dAB, dB),
					 prOutlier=prOutlier[3, ],
					 allele.prob=allele.prob[, c("AA", "AB", "BB")])

	##---------------------------------------------------------------------------
	## copy number 3
	dAAB <- trNormal2(mus[, "AAB"], sds[, "AAB"])
	dABB <- trNormal2(mus[, "ABB"], sds[, "ABB"])
	##tdAAB <- t(dAAB)
	##tdABB <- t(dABB)
	emission[, , 5] <- computeEmitPr(list(dA, dAAB, dABB, dB),
					 prOutlier[5, ],
					 allele.prob[, c("AAA", "AAB", "ABB", "BBB")])

	##---------------------------------------------------------------------------
	## copy number 4
	dAAAB <- trNormal2(mus[, "AAAB"], sds[, "AAAB"])
	dABBB <- trNormal2(mus[, "ABBB"], sds[, "ABBB"])
	emission[, , 6] <- computeEmitPr(list(dA, dAAAB, dAB, dABBB, dB),
					  prOutlier[6, ],
					  allele.prob[, c("AAAA", "AAAB", "AABB", "ABBB", "BBBB")])
	##
	## small p.hom makes homozygous genotypes less informative
	##
	if(p.hom < 1){
		is.hom <- BAF < 0.02 | BAF > 0.98
		for(j in seq_len(nc)){
			i <- which(is.hom[, j])
			if(length(i) > 0)
				emission[i, j, c(3, 5, 6)] <- (1-p.hom)*emission[i, j, 2] + p.hom*emission[i, j, c(3, 5, 6)]
		}
	}
	emission[, , 4] <- emission[, , 2]
	np.index <- which(!is.snp)
	if(length(np.index) > 0) emission[np.index, , ] <- 1
	naemit <- is.na(emission)
	if(any(naemit))
		emission[naemit] <- 1
