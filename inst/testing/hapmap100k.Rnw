...
<<tmpDirectory, eval=FALSE>>=
pathFrom <- "/nexsan/bst2/microarray/SNP/hapmap/100k"
if(!file.exists(pathFrom)){
	stop("dir does not exist")
}
pathTo <- "/tmp/rs100kCrlmm"
if(!file.exists(pathTo)){
	dir.create("/tmp/rs100kCrlmm")
}
@ 

<<copyFiles, eval=FALSE>>=
system(paste("cp -R", pathFrom, "/hind ", pathTo, sep=""))
system(paste("cp -R", pathFrom, "/xba ", pathTo, sep=""))
##uncompress
setwd(pathTo)
setwd("hind")
system("gunzip *.gz")
...
<<independentSamples, eval=FALSE>>=
which <- pd$fid == 0
pd <- pd[which, ]
celfiles <- celfiles[which]
sampleNames(pd) <-celfiles
fullFilenames <- paste(getwd(), celfiles, sep="/")
@ 
...
<<justCrlmm.xba, eval=FALSE>>=
system.time(xba <- justCRLMM(fullFilenames, phenoData=pd, verbose=FALSE))
@ 

Preprocessing the \Robject{hind} platform:

<<justCrlmm.hind, eval=FALSE>>=
setwd(paste(pathTo, "hind", sep="/"))
fullFilenames <- paste(getwd(), celfiles, sep="/")
system.time(hind <- justCRLMM(fullFilenames, phenoData=pd, verbose=FALSE))
@ 
...
hind <- hind[, hind$crlmmSNR > 3]
xba <- xba[, xba$crlmmSNR > 3]
@ 

In order to combine the \Robject{SnpCallSetPlus} objects, the
phenotypic information in the \Robject{phenoData} slots must be the
same.  Here, we simply average the two SNR estimates to give an
overall estimate of the SNR in the two platforms.

<<averageSNR, eval=FALSE>>=
avgSNR <- rowMeans(cbind(hind$crlmmSNR, xba$crlmmSNR))
xba$crlmmSNR <- hind$crlmmSNR <- avgSNR
if(identical(phenoData(xba), phenoData(hind))){
	callset <- combine(hind, xba)
}
@ 
...
save(callset, file="/tmp/rs100kCrlmm/callset.rda", compress=TRUE)
@ 

<<echo=FALSE, eval=FALSE>>=
load("/tmp/rs100kCrlmm/callset.rda")
@ 
...
Next, we sweep out the SNP-specific median intensities and then
recenter to the expected copy number (depends on chromosome):
...
<<echo=FALSE>>=
save(fit.hapmap, file="~/test/VanillaICE/inst/testing/fit.hapmap100k.rda", compress=TRUE)
@ 
