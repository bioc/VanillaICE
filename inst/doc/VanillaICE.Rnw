...
class(hmmpredict)
findBreaks(predictions(hmmpredict), states=states, position=ann[, "position"],
...
<<icePlot, fig=TRUE, width=8, height=5, eval=FALSE>>=
...
<<breaks>>=
findBreaks(x=predictions(fit.ice), states=states(fit.ice),
	   position=position(fit.ice), chromosome=chromosome(fit.ice), sample=sampleNames(fit.ice))
##or
...
params.calls <- new("HmmParameter", 
		    states=states(options))
transitionScale(params.calls) <- scaleTransitionProbability(states(options))
...
gp <- plot(snpset(options), fit.calls)
