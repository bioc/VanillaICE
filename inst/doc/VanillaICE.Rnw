...
params <- new("HmmParameter", 
	      chromosome1, 
	      states=c("D", "N", "L", "A"),
              cn.location=c(1, 2, 2, 3),
	      gte.state=c(0.99, 0.7, 0.99, 0.7))
class(params)
@ 
...
Summary statistics for the breakpoints (more useful when multiple chromosomes and samples are in the \Robject{fit} object)
...
See \cite{Scharpf2007a} for a more complete description of the simulated dataset and the features detected by this HMM.  We may plot the data along with the predictions as follows:

<<vanillaPlot,fig=TRUE,width=8,height=5>>=
if(!identical(featureNames(fit), featureNames(chromosome1))){
	fn <- featureNames(params)
	idx <- match(fn, featureNames(chromosome1))
	chromosome1 <- chromosome1[idx, ]
}
##source("~/madman/Rpacks/SNPchip/R/plotSnp-methods.R")
gp <- new("ParSnpSet")
...
This information is incorporated into the HMM emission probabilities
in the following way:

<<params>>=
params.ice <- new("HmmParameter", 
		  chromosome1, 
		  states=c("D", "N", "L", "A"), 
		  cn.ICE=TRUE,
		  gte.state=c(0.99, 0.75, 0.99, 0.75))
...
R $\times$ S $\times$ C, where $S$ is the number of hidden states, $R$
is the number of rows (SNPs), and $C$ is the number of samples.  To
obtain the emission probabilities for the $ith$ SNP, one may use $[$
to subset.  For instance, the emission probabilities for the 5th SNP
are:
...
creating an instance of the class.  For illustration, one could make
the probability of transitioning from an altered state to a normal
state 10 times as likely as the probability of transitioning between
two altered states by the following commmand:
...
params <- new("HmmParameter", 
	      ...
	      states=c("D", "N", "L", "A"), 
              cn.location=c(1, 2, 2, 3), 
	      SCALE=10)  ##10 times as likely to go back to normal
@ 
...
The output from the HMM is an instance of the \Robject{HmmPredict} class and contains the predicted states as well as the breakpoints  for the different states.
...
breakpoints(fit.cn)
graph.par <- new("ParSnpCopyNumberSet")
...
graph.par <- plotSnp(graph.par, chr1.cn, fit.cn)
...
graph.par <- new("ParSnpCallSet")
graph.par$cytoband.side <- 3
graph.par <- getPar(graph.par, chr1.calls)
graph.par$col.predict <- c("black", "white")
graph.par$ylim <- c(-0.5, 1)
graph.par$add.centromere <- FALSE
@ 

<<callsFigure, fig=TRUE, include=TRUE>>=
plotSnp(graph.par, chr1.calls, fit.calls)
legend(0, -0.1, legend="loss of heterozygosity", fill="black", 
